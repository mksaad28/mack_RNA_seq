...
# Load Scomber japonicus Org Package (NCBI) ----
...
while (x <= length(tables)) {
  # Query tables[x]
  query_result <- dbGetQuery(org.Sjaponicus.eg_dbconn(), paste(
    "SELECT * FROM ",
    noquote(tables[x])
  ))
...
GID2accessions <- merge(sj_list$accessions, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "ACCNUM")]

GID2alias <- merge(sj_list$alias, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "ALIAS")]

GID2chromosomes <- merge(sj_list$chromosome, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "CHR")]

GID2entrez <- merge(sj_list$entrez_genes, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "ENTREZID")]

GID2info <- merge(sj_list$gene_info, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "GENENAME", "SYMBOL")]

GID2refseq <- merge(sj_list$refseq, sj_list$genes,
  by.x = "_id", by.y = "_id",
  all.x = TRUE
)[, c("GID", "REFSEQ")]
...
#' no longer grew larger (30530 UIDs)
#'
...
setwd(paste0(
  script_dir,
  "/Japonicus/SJaponicusFromEggnog/Eggnog Mappings/Eggnog Input/"
))
...
# Collect FASTA sequences for BatchEntrez rejects ----
#' BatchEntrez Protein seems to reject "YP_", "XM_", and "XR_" labeled
#' accessions. "YP_" labeled accessions seem useful to include so those FASTA
#' sequences are collected separately and manually combined before eggNOG
#' mapper processing.
#'
#' On a second look this seems like it may not have been necessary.

# Function to retrieve FASTA sequences based on NCBI Reference Sequences
get_protein_sequence <- function(ref_seq) {
  url <- paste0(
    "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=",
    "protein&id=", ref_seq, "&rettype=fasta&retmode=text"
  )
  seq <- getURL(url)
  return(seq)
}

# Your vector of NCBI Reference Sequences
ref_sequences <- allrefseq[1:13]

# Initialize an empty character vector to store the fasta sequences
fasta_seqs <- character()

# Retrieve protein sequences for each NCBI Reference Sequence
for (ref_seq in ref_sequences) {
  fasta_seq <- get_protein_sequence(ref_seq)
  fasta_seqs <- c(fasta_seqs, fasta_seq)
  Sys.sleep(0.34)
}

# Write the fasta sequences to a file
writeLines(fasta_seqs, "YP_prot_sequences.fasta")
...
#' http://eggnog-mapper.embl.de/job_status?jobname=MM_gg2h0__0

# Process eggNOG Mapping Data ----
#' eggNOG mapper output collected as .emapper.annotations.xlsx file
eggnog <- readxl::read_xlsx(
  paste0(
    script_dir, "/Japonicus/SJaponicusFromEggno",
    "g/Eggnog Mappings/Eggnog Output/MM_gg2h0__0.emapper.annotations.xlsx"
  ),
  skip = 2
)
...
remove.packages(pkgs = "org.Sjaponicus.eg.db")
...
# Collect org.Sjaponicus.eg.db as an R list() to visualize proper install
tables <- dbListTables(org.Sjaponicus.eg_dbconn())
