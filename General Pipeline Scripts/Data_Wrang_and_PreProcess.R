...
# Read in Thunnus maccoyii gene metadata ----
  gene_lookup <- 
    read_tsv("data/michael_saad/gene_result.txt", 
...
# Filter metadata by GeneSymbol ----
 # Necessary since STAR results are outputted by GeneSymbol and not by GeneID
  gene_lookup_filtered <- 
    gene_lookup |>
    select(Symbol, description,GeneID) |>
    group_by(Symbol) |>
    filter(!duplicated(Symbol)) |>
    ungroup()
    # filter(!duplicated(Symbol)) assures a 1:1 mapping even when there are 
    # multiple entries for 1 ensembl ID
  
  gene_id_symbol <- gene_lookup_filtered |> select(GeneID, Symbol)
    # Used to convert GeneSymbols to GeneIDs after DESeq2
...
# Featurecounts Results Wrangling ----
...
# Create sample_lookup ----
  ...
  sample_lookup <- expr_long |>
    ...
    unique() |>
    separate(sample, 
             into = c("passage", "protocol_replicate"), 
             remove = FALSE) |>
    mutate(
