...
```{r Eggnog}
a <- 
  readLines("data/michael_saad/GCF_910596095.1/cds_from_genomic.accessions" + 
                 "_only.fna") # Read in fasta file
...
gene_lookup_abridged <- gene_lookup |> select(GeneID, description) 
...
expr_wide_duplicated <-merge(expr_wide, duplicated_symbol, 
                             by.x=c("gene_symbol"), by.y=c("Symbol"))
